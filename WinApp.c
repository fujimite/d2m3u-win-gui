...
#define DARK_EDIT_BG     RGB( 22,  32,  41)
#define DARK_TEXT        RGB(220, 220, 220)
#define DARK_BTN_BG      RGB( 55,  55,  55)
#define DARK_BTN_HOV     RGB( 72,  72,  72)
#define DARK_BTN_BORDER  RGB( 90,  90,  90)
#define DARK_MENU_BG     RGB( 30,  30,  30)
#define DARK_MENU_HOV    RGB( 60,  60,  60)
#define DARK_MENU_TEXT   RGB(210, 210, 210)

#ifndef DWMWA_USE_IMMERSIVE_DARK_MODE
#define DWMWA_USE_IMMERSIVE_DARK_MODE 20
#endif

#define REG_KEY   L"Software\\d2m3u"
#define REG_VALUE L"DarkMode"
...
static const WCHAR* g_menuLabels[] = { L"&File", L"&View", L"&Help" };
...
static void SetButtonOwnerDraw(HWND hBtn, BOOL ownerDraw) {
    LONG_PTR style = GetWindowLongPtrW(hBtn, GWL_STYLE);
    style = ownerDraw
        ? (style & ~BS_PUSHBUTTON) | BS_OWNERDRAW
        : (style & ~BS_OWNERDRAW) | BS_PUSHBUTTON;
    SetWindowLongPtrW(hBtn, GWL_STYLE, style);
    InvalidateRect(hBtn, NULL, TRUE);
}

static void UpdateButtonStyles(BOOL dark) {
    HWND buttons[] = { hBtnBrowse, hBtnSave, hBtnGenerate };
    for (int i = 0; i < 3; i++)
        if (buttons[i]) SetButtonOwnerDraw(buttons[i], dark);
}
...
static void DrawDarkButton(LPDRAWITEMSTRUCT dis) {
    ...
    HBRUSH hBr = CreateSolidBrush(pressed ? DARK_BTN_HOV : DARK_BTN_BG);
    FillRect(hdc, &rc, hBr);
    DeleteObject(hBr);
...
static void ApplyTheme(HWND hWnd) {
    ...
    if (bDarkMode) {
        hBgBrush = CreateSolidBrush(DARK_BG);
        hEditBrush = CreateSolidBrush(DARK_EDIT_BG);
    }
    else {
        hBgBrush = CreateSolidBrush(GetSysColor(COLOR_BTNFACE));
        hEditBrush = CreateSolidBrush(GetSysColor(COLOR_WINDOW));
    }

    SetClassLongPtrW(hWnd, GCLP_HBRBACKGROUND, (LONG_PTR)hBgBrush);
    ApplyDarkTitleBar(hWnd, bDarkMode);

    HMENU hBar = GetMenu(hWnd);
    if (hBar) {
        SetMenuOwnerDraw(hBar, bDarkMode);
        DrawMenuBar(hWnd);
    }

    UpdateButtonStyles(bDarkMode);
...
static HWND MakeLabel(HWND hWnd, const WCHAR* text, int x, int y, int w, int h) {
    HWND hLabel = CreateWindowW(L"STATIC", text,
        WS_VISIBLE | WS_CHILD, x, y, w, h, hWnd, NULL, hInst, NULL);
...
static BOOL BrowseForFolder(HWND hWnd, WCHAR* outPath, DWORD outSize) {
    ...
    if (FAILED(CoCreateInstance(&CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER,
        &IID_IFileOpenDialog, (void**)&pDlg))) return FALSE;
...
static BOOL BrowseForSaveFile(HWND hWnd, WCHAR* outPath, DWORD outSize) {
    ...
    if (FAILED(CoCreateInstance(&CLSID_FileSaveDialog, NULL, CLSCTX_INPROC_SERVER,
        &IID_IFileSaveDialog, (void**)&pDlg))) return FALSE;
...
static void CreateLogWindow(HWND hParent) {
    ...
    hWndLog = CreateWindowW(L"LogWindow", L"d2m3u \u2013 Log",
        WS_OVERLAPPEDWINDOW, 150, 150, 600, 400, hParent, NULL, hInst, NULL);

    hEdtLog = CreateWindowW(L"EDIT", L"",
        WS_VISIBLE | WS_CHILD | WS_VSCROLL | WS_HSCROLL |
        ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL | ES_AUTOHSCROLL,
        0, 0, 600, 400, hWndLog, NULL, hInst, NULL);

    HFONT hMono = CreateFontW(-14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        DEFAULT_QUALITY, FIXED_PITCH | FF_MODERN, L"Lucida Console");
...
static LRESULT CALLBACK LicenseWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        HWND hTxt = CreateWindowW(L"STATIC",
            ...
            WS_VISIBLE | WS_CHILD, 10, 10, 380, 120, hWnd, NULL, hInst, NULL);
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == BTN_CURL_LIC)   OpenLicenseFile(hWnd, L"CURL_LICENSE.txt");
        if (LOWORD(wParam) == BTN_FFMPEG_LIC) OpenLicenseFile(hWnd, L"FFMPEG_LICENSE.txt");
        break;
    case WM_CLOSE:
...
static void ShowLicenseWindow(HWND hParent) {
    ...
    HWND hWnd = CreateWindowW(L"LicenseWindow", L"Licensing",
        ...
        200, 200, 410, 188, hParent, NULL, hInst, NULL);
...
static LRESULT CALLBACK AboutWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        if (hBmpOrig) {
            BITMAP bm; GetObject(hBmpOrig, sizeof(bm), &bm);
            ...
            ReleaseDC(NULL, hdcScreen); DeleteObject(hBmpOrig);
        }
        HFONT hF = (HFONT)GetStockObject(DEFAULT_GUI_FONT);
        HWND  hTxt = CreateWindowW(L"STATIC",
            L"d2m3u GUI for Windows\r\nprod. fujimite\r\n\r\n"
            L"Generates an m3u playlist from a local or web directory.",
            WS_VISIBLE | WS_CHILD, 10, 10, 380, 80, hWnd, NULL, hInst, NULL);
        SendMessage(hTxt, WM_SETFONT, (WPARAM)hF, TRUE);
        break;
    }
    case WM_PAINT: {
        ...
        if (hBmpScaled) {
            ...
            BitBlt(hdc, (rc.right - imgW) / 2, 60, imgW, imgH, hdcMem, 0, 0, SRCCOPY);
...
static void ShowAboutWindow(HWND hParent) {
    ...
    HWND hWnd = CreateWindowW(L"AboutWindow", L"About d2m3u",
        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU,
        200, 200, 410, 320, hParent, NULL, hInst, NULL);
    ShowWindow(hWnd, SW_SHOW);
    UpdateWindow(hWnd);
}
...
static void RunBase(HWND hWnd) {
    ...
    if (wcslen(input) == 0) {
        MessageBoxW(hWnd, L"Please enter an input folder or URL.",
            L"Missing Input", MB_ICONWARNING | MB_OK);
        goto cleanup;
    }
    if (wcslen(output) == 0) {
        MessageBoxW(hWnd, L"Please enter an output file path.",
            L"Missing Output", MB_ICONWARNING | MB_OK);
        goto cleanup;
    }
    ...
    if (embedAuth) wcsncat_s(cmd_buf, 4096, L" -e", _TRUNCATE);
    if (splitDir)  wcsncat_s(cmd_buf, 4096, L" -s", _TRUNCATE);
    ...
    _snwprintf_s(pos_buf, MAX_PATH * 2 + 8, _TRUNCATE,
        L" %s %s", quoted_input, quoted_output);
    wcsncat_s(cmd_buf, 4096, pos_buf, _TRUNCATE);

    if (bLogVisible) { LogPrint(L"Running: "); LogPrint(cmd_buf); LogPrint(L"\r\n"); }
    ...
    while (ReadFile(hReadPipe, buf, sizeof(buf) - 1, &bytesRead, NULL) && bytesRead > 0) {
        ...
        while (PeekMessage(&pumpMsg, NULL, 0, 0, PM_REMOVE)) {
            TranslateMessage(&pumpMsg); DispatchMessage(&pumpMsg);
        }
    }
    ...
    if (exit_code == 0) {
        MessageBoxW(hWnd, L"Playlist generated successfully.",
        ...
        MessageBoxW(hWnd, emsg, L"Error", MB_ICONERROR | MB_OK);
        if (!bLogVisible) {
            bLogVisible = TRUE;
...
cleanup:
    free(input); free(output); free(username); free(password);
    free(bin_dir); free(bin_path); free(cmd_buf);
    free(quoted_input); free(quoted_output); free(quoted_bin); free(pos_buf);
}
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        AppendMenuW(hViewMenu,
            MF_STRING | (bDarkMode ? MF_CHECKED : MF_UNCHECKED),
            IDM_DARKMODE, L"&Dark Mode");
        ...
        hEdtInput = CreateWindowW(L"EDIT", L"",
            ...
            x, y, w - 40, eh, hWnd, (HMENU)EDT_INPUT, hInst, NULL);
        SendMessage(hEdtInput, WM_SETFONT, (WPARAM)hFont, TRUE);
        hBtnBrowse = CreateWindowW(L"BUTTON", L"...",
            WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON | WS_TABSTOP,
            x + w - 36, y, 30, eh, hWnd, (HMENU)BTN_BROWSE, hInst, NULL);
        ...
        hEdtUsername = CreateWindowW(L"EDIT", L"",
            WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL | WS_TABSTOP,
            x, y, w - 7, eh, hWnd, (HMENU)EDT_USERNAME, hInst, NULL);
        ...
        hEdtPassword = CreateWindowW(L"EDIT", L"",
            ...
            x, y, w - 7, eh, hWnd, (HMENU)EDT_PASSWORD, hInst, NULL);
        ...
        hChkEmbedAuth = CreateWindowW(L"BUTTON", L"Embed authentication",
            ...
            x, y, w, eh, hWnd, (HMENU)CHK_EMBEDAUTH, hInst, NULL);
    ...
    case WM_DRAWITEM: {
        ...
        if (dis->CtlType == ODT_BUTTON && bDarkMode) {
            DrawDarkButton(dis);
            return TRUE;
        }

        if (dis->CtlType == ODT_MENU && bDarkMode) {
            BOOL sel = (dis->itemState & ODS_SELECTED) != 0;
            HBRUSH hBr = CreateSolidBrush(sel ? DARK_MENU_HOV : DARK_MENU_BG);
            FillRect(dis->hDC, &dis->rcItem, hBr);
            DeleteObject(hBr);

            int          idx = (int)(dis->itemData);
            const WCHAR* label =
                (idx >= 0 && idx < (int)ARRAYSIZE(g_menuLabels))
                ? g_menuLabels[idx] : L"";

            SetBkMode(dis->hDC, TRANSPARENT);
            SetTextColor(dis->hDC, DARK_MENU_TEXT);
            HFONT hOld = SelectObject(dis->hDC, hFont);
    ...
    case WM_COMMAND:
        ...
        case EDT_INPUT:
            if (HIWORD(wParam) == EN_CHANGE) UpdateAuthFields(hWnd);
            break;
        case BTN_BROWSE: {
            WCHAR path[MAX_PATH] = { 0 };
            if (BrowseForFolder(hWnd, path, MAX_PATH)) SetWindowTextW(hEdtInput, path);
            break;
        }
        case BTN_SAVE: {
            WCHAR path[MAX_PATH] = { 0 };
            if (BrowseForSaveFile(hWnd, path, MAX_PATH)) SetWindowTextW(hEdtOutput, path);
            break;
        }
        case BTN_GENERATE:
            RunBase(hWnd);
        ...
        case IDM_DARKMODE: {
            ...
            MENUITEMINFOW mii = { sizeof(mii) };
            mii.fMask = MIIM_STATE;
            mii.fState = bDarkMode ? MFS_CHECKED : MFS_UNCHECKED;
            SetMenuItemInfoW(hViewMenu, IDM_DARKMODE, FALSE, &mii);
            SaveDarkModePref(bDarkMode);
            ApplyTheme(hWnd);
            break;
        }
        case IDM_LICENSING: ShowLicenseWindow(hWnd); break;
        case IDM_ABOUT:     ShowAboutWindow(hWnd);   break;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrev,
    ...
    HWND hWnd = CreateWindowW(L"MainWindow", L"d2m3u",
        ...
        100, 100, 525, 400, NULL, NULL, hInst, NULL);
    ...
    if (hBar) {
        ...
        for (int i = 0; i < count; i++) {
            MENUITEMINFOW mii = { sizeof(mii) };
            mii.fMask = MIIM_DATA;
            mii.dwItemData = (ULONG_PTR)(INT_PTR)i;
            SetMenuItemInfoW(hBar, i, TRUE, &mii);
        }
    }
